...
- [ ] **S02: Toggle + Layout** `risk:medium` `depends:[S01]`
...
- [ ] **S03: Enrichment + Theme** `risk:low` `depends:[S02]`
  > After this: Running sessions show tab count, current session shows active pane command, info verbosity is configurable (minimal/full via KDL config), and all colours match Catppuccin Frappe with semantic status indicators (green=running, dim=stopped, yellow=exited).
